...
public class CompanyLCLimitDetailActivity extends BaseActivity {
    private Context mContent;
    private RequestQueue mRequestQueue;
    private final int LcLimitType = 2; //查看批复时的type
    ...
    private void initToolbar() {
        ...
        tv_title.setText(getStrings(R.string.toolbar_company_LCLimit_Detail));
    }

    @Override
    public void onAfterViews() {
        mContent = CompanyLCLimitDetailActivity.this;
        mRequestQueue = Volley.newRequestQueue(mContent);
        LCId = getIntent().getIntExtra(AppDelegate.CUSTOMER_ID_COMPANY, 0);
    ...
    public void onBackgrounds() {
        final String url = MoreUserDal.GetServerUrl() + "/api/CreditGuarantee/CompanyLCLimitDetails?lcId=" + LCId;
        LogUtils.d("限额的详情" + url);
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_reply:
                ...
                CompanyReplysActivity_.intent(mContent).extra(AppDelegate.REPLYSKEYWORD, applyKeyword)
                        .extra(AppDelegate.TYPE, LcLimitType).start();
                break;
        }
