package com.example.ZMTCSD.view;
...
public class AndroidSegmentedControlView extends RadioGroup {
    ...
    public AndroidSegmentedControlView(Context context, AttributeSet attrs) throws Exception {
        ...
        //Here's where overwrite the defaults with the values from the xml attributes 这里覆盖默认值从xml属性
        ...
        try {
            ...
            //Item and value arrays need to be of the same length 长度
    ...
    private void update() {
        ...
        while (itemIterator.hasNext()) {
            ...
            if (i == 0) {
                //Left
                GradientDrawable leftUnselected = (GradientDrawable) mCtx.getResources().getDrawable(R.drawable.shape_segmentview_left_option).mutate();
                leftUnselected.setStroke(twoDP, selectedColor);

                leftUnselected.setColor(unselectedColor);
                GradientDrawable leftSelected = (GradientDrawable) mCtx.getResources().getDrawable(R.drawable.shape_segmentview_left_option_selected).mutate();
                ...
                //Right
                GradientDrawable rightUnselected = (GradientDrawable) mCtx.getResources().getDrawable(R.drawable.shape_segmentview_right_option).mutate();
                rightUnselected.setStroke(twoDP, selectedColor);

                rightUnselected.setColor(unselectedColor);
                GradientDrawable rightSelected = (GradientDrawable) mCtx.getResources().getDrawable(R.drawable.shape_segmentview_right_option_selected).mutate();
                ...
                //Middle
                GradientDrawable middleUnselected = (GradientDrawable) mCtx.getResources().getDrawable(R.drawable.shape_segmentview_middle_option).mutate();
                middleUnselected.setStroke(twoDP, selectedColor);
                middleUnselected.setDither(true);
                middleUnselected.setColor(unselectedColor);
                GradientDrawable middleSelected = (GradientDrawable) mCtx.getResources().getDrawable(R.drawable.shape_segmentview_middle_option_selected).mutate();
    ...
    /**
     * Get currently selected segment and the view identifier  得到当前选中视图标识符
     *
    ...
    /**
     * Get currently selected segment  得到当前选择的部分
     *
     ...
     * default selection 每一部分中设置项目和值。还提供了一个辅助设置 默认
     *
     * @param items
     * @param values
     ...
     * onSelectionChanged mListener 使用多个视图 标示符
     *
     * @param identifier
     */
    ...
    /**
     * Set to true if you want each segment to be equal width 设置为真,如果你想要每个段宽度相等
     *
     * @param equalWidth
     */
