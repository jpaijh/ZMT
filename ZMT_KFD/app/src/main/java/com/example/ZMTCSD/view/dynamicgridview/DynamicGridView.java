package com.example.ZMTCSD.view.dynamicgridview;
...
public class DynamicGridView extends GridView {
    ...
    private void startWobbleAnimation() {
        for (int i = 0; i < getChildCount(); i++) {
            View v = getChildAt(i);
            if (v != null && Boolean.TRUE != v.getTag(R.id.wobble_tag)) {
                if (i % 2 == 0)
                    animateWobble(v);
                else
                    animateWobbleInverse(v);
                v.setTag(R.id.wobble_tag, true);
            }
        }
    }
    ...
    private Boolean isShowEmptyItemLines = false;

    /**
     * @param isShowLines          是否显示网格线,默认不显示
     * @param isShowEmptyItemLines 是否显示空item的网格线,默认不显示
     */
    public void setShowLines(Boolean isShowLines, Boolean isShowEmptyItemLines) {
        this.isShowLines = isShowLines;
        this.isShowEmptyItemLines = isShowEmptyItemLines;
    }
     ...
     * @param canvas
     */
    private void drawGridViewLines(Canvas canvas) {
        if (getChildAt(0) != null) {
            int itemWidth = getChildAt(0).getWidth(); // 第一个item宽度
            int column = getWidth() / itemWidth; // GridView的列数
            int itemCounts = getChildCount(); // 不为空的总item数

            Paint mPaint = new Paint();
            mPaint.setStyle(Paint.Style.STROKE);
            mPaint.setStrokeWidth(getResources().getInteger(R.integer.gridView_line_width));
            mPaint.setColor(getResources().getColor(R.color.gridView_line_color));

            for (int i = 0; i < itemCounts; i++) { // 遍历所有不为空的item
                View currentItem = getChildAt(i);
                float startX = currentItem.getLeft();
                float endX = currentItem.getRight();
                float startY = currentItem.getTop();
                float endY = currentItem.getBottom();

                if ((i + 1) % column == 0) { // 每行最右边item
                    canvas.drawLine(startX, endY, endX, endY, mPaint); // 画底下线
                } else {  // 其他item
                    canvas.drawLine(startX, endY, endX, endY, mPaint); // 画底下线
                    canvas.drawLine(endX, startY, endX, endY, mPaint); // 画右边线
                }
            }

            if (isShowEmptyItemLines)
                drawGridViewEmptyItemLines(canvas, column, itemCounts, mPaint, itemWidth);
        }
    }
     ...
     * @param mPaint
     */
    private void drawGridViewEmptyItemLines(Canvas canvas, int column, int itemCounts, Paint mPaint, int itemWidth) {
        int number = itemCounts % column;
        if (number != 0) { // 判断有没有空item
            View lastItem = getChildAt(itemCounts - 1); // 最后一个有内容的item,根据它来计算其他空item Y坐标
            float startY = lastItem.getTop();
            float endY = lastItem.getBottom();

            int nullItems = column - number; // 计算有多少个空item
            for (int j = 1; j <= nullItems; j++) {
                if (j == nullItems) { // 画最后一个空item
                    canvas.drawLine(itemWidth * (column - 1), endY, itemWidth * column, endY, mPaint); // 画底下横线
                } else { // 画中间的空items
                    canvas.drawLine(itemWidth * (number + j), startY, itemWidth * (number + j), endY, mPaint); // 画右边横线
                    canvas.drawLine(itemWidth * (number + j - 1), endY, itemWidth * (number + j), endY, mPaint); // 画底下横线
                }
            }
        }
    }
